...
RANK = 24    # Low rank prevents memorization
ALPHA = 4    # Alpha < Rank = More stable, less aggressive
...
class RobustInpaintDataset(Dataset):
    ...
    def simulate_mask_failure(self, mask):
        """
        Simulates the detection model failing to catch all bars.
        It randomly draws BLACK lines/rectangles over the WHITE mask.
        """
        # 1. Convert to RGB to use Draw (though L works, RGB is safer for some PIL versions)
        mask_draw = mask.convert("RGB")
        draw = ImageDraw.Draw(mask_draw)
        w, h = mask.size
        
        # 2. Randomly decide failure type
        # Type A: "Slice" failure (Missing a horizontal/vertical bar)
        if random.random() > 0.5:
            num_cuts = random.randint(1, 3)
            for _ in range(num_cuts):
                # Random thickness and position
                ...
                # Draw black box (Un-masking this area)
                draw.rectangle([x1, y1, x1+cut_w, y1+cut_h], fill=(0,0,0))
        return mask_draw.convert("L")
    ...
    def __getitem__(self, idx):
        ...
        if self.augment_mask:
            if random.random() > 0.5:
                mask = self.simulate_mask_failure(mask)                
        ...
        return {
            ...
            "masked_image_values": self.img_tf(censored), # Censored image still has the FULL black bars
            "filename": fn
        }
